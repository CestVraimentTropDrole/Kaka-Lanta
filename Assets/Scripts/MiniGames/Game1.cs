...
public class F1LightsMinigame : MonoBehaviour
{
    ...
    [SerializeField] private float lightOnDelay = 1f;
    ...
    void Start()
    {
        ...
        hasReacted = new bool[players.Count];
    ...
    private IEnumerator StartGameSequence()
    {
        ...
        {
            if (light != null)
                light.color = new Color(0.1f, 0.1f, 0.1f);
        }
    ...
    private void CheckPlayerInputs()
    {
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            bool pressed =
                ...
                RFIDManager.instance.IsButtonPressed(i);

            if (pressed)
            {
                Debug.Log($"INPUT joueur {i + 1}");
                RegisterReaction(i);
            }
        }

        // Fin si tous ont réagi
        if (currentState == GameState.Racing)
        {
            bool allReacted = true;
            for (int i = 0; i < hasReacted.Length; i++)
            {
                if (!hasReacted[i])
                {
                    allReacted = false;
